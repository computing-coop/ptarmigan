...
describe SessionsController do
  ...
  describe "logout_killing_session!" do
    ...
    it 'nils the current applicant'      do logout_killing_session!; current_applicant.should be_nil end
    it 'kills :applicant_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:applicant_id, nil).at_least(:once)
      logout_killing_session!
    end
    it 'forgets me' do    
      current_applicant.remember_me
      current_applicant.remember_token.should_not be_nil; current_applicant.remember_token_expires_at.should_not be_nil
      Applicant.find(1).remember_token.should_not be_nil; Applicant.find(1).remember_token_expires_at.should_not be_nil
      logout_killing_session!
      Applicant.find(1).remember_token.should     be_nil; Applicant.find(1).remember_token_expires_at.should     be_nil
    end
  end
  ...
  describe "logout_keeping_session!" do
    ...
    it 'nils the current applicant'      do logout_keeping_session!; current_applicant.should be_nil end
    it 'kills :applicant_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:applicant_id, nil).at_least(:once)
  ...
  describe "Logging in by cookie" do
    def set_remember_token token, time
      @applicant[:remember_token]            = token; 
      @applicant[:remember_token_expires_at] = time
      @applicant.save!
    end    
    before do 
      @applicant = Applicant.find(:first); 
